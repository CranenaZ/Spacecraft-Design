...
void pack(uint32_t v,int b,int &p){while(b--){int i=p>>3,bit=7-(p&7);pkt[i]|=((v>>b)&1)<<bit;p++;}}
...
void build(){
  ...
  for(int i=0;i<3;i++){pack(ms[i]<0,1,p);pack(min(127,int(abs(ms[i])*3)),7,p);}

  Wire.beginTransmission(0x68);Wire.write(0x3B);Wire.endTransmission(false);
  Wire.requestFrom(0x68,14,true);
  int16_t d[6];
  for(int i=0;i<3;i++){d[i]=(Wire.read()<<8)|Wire.read();Wire.read();Wire.read();}
  ...
  for(int i=0;i<6;i++){float v=d[i]*ss[i];pack(v<0,1,p);pack(min(127,int(abs(v)*3)),7,p);}
  ...
  if(gps.location.isUpdated()){
    ...
    auto ec=[&](double x,int w,int f){pack(x<0,1,p); if(x<0)x=-x; pack(uint32_t(x),w,p); pack(uint32_t((x-int(x))*(1UL<<f)),f,p);};
...
void loop(){
  if(millis()-lastT>3000){
    ...
    build();
    toHex();
    Serial.println(hexOut);  // Existing line
    ...
    // Extract hex characters 19 to 23 (i.e., hexOut[18] to hexOut[22])
    char poopyOut[11]; // "POOPY" + 5 chars + null terminator
    strcpy(poopyOut, "POOPY");
    strncpy(poopyOut + 5, hexOut + 18, 5); // Copy characters 18 to 22
    poopyOut[10] = '\0'; // Null-terminate the string
    Serial.println(poopyOut); // Print the "POOPY_____" line
  }
}